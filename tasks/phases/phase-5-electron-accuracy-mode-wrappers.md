...
- [x] Step 5.8: [automated] Run Phase 5 verification: `npm run typecheck`, `npm test`, `npm run build`, and wrapper setup renderer smoke tests.
  ...
  **Result for Step 5.8:**
  - Phase 5 verification passed from `electron-app/`: `npm run typecheck`, `npm test -- --run`, `npm run build`, and `npm run smoke:electron`.
  - Full Vitest passed with 31 test files and 132 tests. The production build passed main, preload, and renderer bundle generation.
  - Electron smoke passed all route-level mocked local usage states: popover disabled/configured/ready GitHub states, settings, onboarding, overlay compact/minimal/sidebar, and settings error retry.
  - Accepted warning: Node's experimental SQLite warning during storage/integration tests. No unexpected warnings were emitted.

## Milestone
- [x] Accuracy Mode is explicit, opt-in, verifiable, and reversible.
- [x] Wrappers persist only derived metadata and never raw prompts, stdout, session keys, GitHub tokens, or provider auth tokens.
- [x] Wrapper events improve confidence where justified.
- [x] All phase tests pass.
- [x] No regressions.

## On Completion
Phase 5 completed 2026-04-19. Codex and Gemini Accuracy Mode wrappers are generated under app-owned user-data paths, verified through harmless CLI probes, persisted through a derived-metadata-only SQLite wrapper event ledger, and merged into provider confidence without weakening passive-only support. Settings and onboarding expose optional, reversible setup with manual shell instructions, troubleshooting, removal instructions, and privacy copy. The two real-shell wrapper setup checks remain non-blocking human follow-ups.
